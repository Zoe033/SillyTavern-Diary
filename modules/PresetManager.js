...
export class PresetManager {
  ...
  async getDiarySettings() {
    try {
      if (!this.settings) {
        // 使用正确的扩展设置访问方式
        const extensionSettings = typeof extension_settings !== 'undefined' 
          ? extension_settings 
          : window.extension_settings || {};
          
        this.settings = extensionSettings[this.extensionName] || {};
  ...
  async saveDiarySettings(settings) {
    try {
      ...
      extensionSettings[this.extensionName] = this.settings;
      
      // 使用正确的保存方法
      if (typeof saveSettingsDebounced !== 'undefined') {
        await saveSettingsDebounced();
      } else if (window.saveSettingsDebounced) {
        await window.saveSettingsDebounced();
      } else {
        // 备用保存方式
        localStorage.setItem('SillyTavern_extensionSettings', JSON.stringify(extensionSettings));
      }
